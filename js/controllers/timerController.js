...
export class TimerController {
    constructor() {
        this.dom = {
            ...
            resetButton: null,
            rankingForm: null,
            playerNoteInput: null,
            uploadRankingBtn: null,
            viewRankingBtn: null
        };
    ...
    _bindEvents() {
        ...
        if (this.dom.rankingForm) {
            this.dom.rankingForm.addEventListener('submit', (e) => {
                e.preventDefault();
                this.saveToLocalRanking();
            });
        }

        if (this.dom.viewRankingBtn) {
            this.dom.viewRankingBtn.addEventListener('click', () => {
                window.Modal?.alert('排行榜功能已移除，仅保留本地保存记录。', '提示');
            });
        }
    }
    ...
    saveToLocalRanking() {
        ...
        try {
            ...
            if (this.dom.playerNoteInput) this.dom.playerNoteInput.value = '';
        } catch (error) {
            window.Modal?.alert(`保存失败: ${error.message}`, '错误');
        } finally {
            if (this.dom.uploadRankingBtn) {
                this.dom.uploadRankingBtn.disabled = false;
                this.dom.uploadRankingBtn.innerHTML = '<i class="fas fa-save"></i> 保存到本地';
            }
        }
    }
